...
def signals(text_en):
    ...
    return {"specific": {canon(p) for p in raw},
            ...
            "actors": {a for a in ACTORS if a in t},
...
def grade_event(n_sides, n_reports):
    ...
    if n_reports == 2: return "C — repeated within one side"
...
def main():
    ...
    print(f"\n{len(msgs)} messages -> {len(events)} events", file=sys.stderr)
    ...
    for e in (gA + gB)[:10]:
        ...
        print(f"  [{e['grade'][:1]}] {loc} ({e['zone']}){var} - {e['n_reports']}rpt/"
